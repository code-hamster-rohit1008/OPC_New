...
async function fetchAvailableTimeSlots(selectedDate) {
    try {
        const response = await fetch(
            `https://opc-backend-i4tf.onrender.com/meet/timings?date=${selectedDate.toISOString().split('T')[0]}`
        );
...
async function sendScheduleMeetToServer(formData) {
    try {
        ...
        const response = await fetch("https://opc-backend-i4tf.onrender.com/meet/schedule", {
            method: "POST",
            headers: {
